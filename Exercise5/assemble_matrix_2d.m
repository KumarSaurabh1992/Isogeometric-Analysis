...
%         for l = 1:nquad
        ...
        N = bspline_basisfuns((i0),xi1,p,nurb.knots{1});
        M = bspline_basisfuns((j0),eta1,q,nurb.knots{2});
...
%         
        if (use_bspline)
            ...
            S = bspline_eval(bspline,coeffs, nshp, u2D);
            % derivatives in the reference domain
            dS_ref = bspline_derv_eval(bspline,coeffs,nshp,u2D);
        else
...
%                 Use the '/' operator to compute the left inverse of the jacobian.
                K = reshape(dS_phys(j,:,i),2,1);
                dS_phys(j,:,i) = K + ((1\(dF(:,:,i)))*reshape(dS_ref(j,:,i),2,1));
            end
        end
%         
